...
var util = {
  ...
  */
  ...
  isNotIsoDateString: function (ds) {
    ...
    if (!util.isInteger(YYYY) || YYYY<1000 || YYYY>9999) return "YYYY out of range!";
    if (!util.isInteger(MM) || MM<1 || MM>12) return "MM out of range!";
    if (!util.isInteger(DD) || DD<1 || DD>31) return "MM out of range!";
    return "";
  },
  /**
  ...
  /**
   * Create a list element from an associative array of objects
   *
   ...
   * @param {object} selectionRange  An associative array of objects, which is used to 
   *                 create the options of the select element
   * @param {object} selection  An associative array of objects, which is used to
